...
class VOCAugDataSet(Dataset):
    ...
    def __init__(self, dataset_path='/home/mec-lab/LBX/Codes_for_Lane_Detection/ENet-Label-Torch/data/CULane/list', data_list='test_img', transform=None):
        #print(os.path.join(dataset_path, data_list + '.txt'))
    ...
    def __getitem__(self, idx):
        ...
        label = label.squeeze()
        if self.transform:
            image, label = self.transform((image, label)) # (208, 976, 3)
            #print(image.shape)
            image = torch.from_numpy(image).permute(2, 0, 1).contiguous().float()
            label = torch.from_numpy(label).contiguous().long()
        if self.is_testing:
            #print(image.shape) # torch.Size([3, 208, 976])
            return image, label, self.img[idx]
        else:
            return image, label, exist